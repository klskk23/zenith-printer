...
describe('renaming a template', () => {
  it('mounts and actually renders the library', async () => {
    expect(() => render(wrap(<LabelsPage />))).not.toThrow()
  ...
  it('opens a field carrying the current name, so a small edit is a small edit', async () => {
    ...
    expect((screen.getByLabelText('标签名称') as HTMLInputElement).value).toBe('面单')
  })
  ...
  it('sends the new name and nothing else', async () => {
    ...
    await screen.findByText('面单')
    fireEvent.click(screen.getByRole('button', { name: '改名' }))
    fireEvent.change(screen.getByLabelText('标签名称'), { target: { value: '快递面单' } })
  ...
  it('trims the name, so a stray space is not part of it', async () => {
    ...
    await screen.findByText('面单')
    fireEvent.click(screen.getByRole('button', { name: '改名' }))
    fireEvent.change(screen.getByLabelText('标签名称'), { target: { value: '  面单 2  ' } })
    fireEvent.click(screen.getByRole('button', { name: '保存' }))
...
describe('the tile paper', () => {
  // Drawn from the label's own content, not fetched: what is left to check is
  // the paper's shape and that it is sized before anything arrives.
