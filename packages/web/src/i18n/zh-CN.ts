...
export const copy = {
  ...
  dataSources: {
    ...
    columnChangeAffected: '下面这些标签引用了消失的列，应用之后它们会取不到值：',
    columnChangeNoneAffected: '目前没有标签引用消失的列。',
    ...
    googlePreviewHint: '列名就是标签里 ${} 引用的名字。若下面显示的不是你要的列名，多半是表头不在首行——回 Google 调整后再来。',
    ...
    deleteWarning: '表里的行会被删掉，无法恢复。引用它的标签不会被删，但会显示警告——把它们重新绑到另一张同形状的表即可复原。',
    deleteAffected: (names: string[]): string => `正在引用它的标签：${names.join('、')}`,
    deleteConfirm: '确认删除',
    replaceTitle: '替换后有列会消失',
    replaceWarning: (columns: string[], templates: string[]): string =>
      `新文件里没有这些列：${columns.join('、')}。以下标签引用了它们，替换后会断掉：${templates.join('、')}`,
    replaceConfirm: '仍然替换',
    addRow: '加一行',
    deleteRow: '删除此行',
    ...
    rebindHint: '在标签的属性栏里重新选择一张同形状的表即可复原',
    menu: {
      cut: '剪切',
      copy: '复制',
      paste: '粘贴',
  ...
  pools: {
    ...
    resetConfirm: '确认重置',
    deleteTitle: (name: string): string => `删除「${name}」？`,
    deleteWarning: '已发放的号段会保留在任务记录里，因为那些号码印在实物上。仍被标签引用的序号池不能删除。',
    deleteConfirm: '确认删除',
    deleteRefused: '还有标签正在引用它。请先把那些标签改用其他序号池，或改为常量。',
  },
  variables: {
    heading: '变量',
    ...
    tempEmptyCell: '（空）',
    tempHint: '只改变画布上显示的内容，不写进标签，也不影响打印',
  },
  ...
  templates: {
    ...
    searchPlaceholder: '搜索标签名称',
    open: '打开',
    confirmDelete: '确定删除这张标签吗？已打印的历史记录不受影响。',
    thumbnailAlt: (name: string): string => `${name} 的缩略图`,
    thumbnailMissing: '这张标签画不出预览图',
    import: '导入',
    importFile: '选择标签文件',
    export: '导出',
    exportAll: '导出全部',
    importUnreadable: '这个文件不是有效的 JSON，读不出来。请确认选的是从「标签 - 导出全部」得到的文件。',
    importConflictTitle: '文件里有本机已存在的标签',
    importConflictBody: '下面这些标签本机已经有了。覆盖会用文件里的版本替换现有的，且无法撤销；另存副本则两份都保留。',
  ...
  settings: {
    ...
    pruneImagesHint: '粘贴到标签里的图片会立刻上传，被丢弃的粘贴就留在了服务器上。这里清掉没有任何标签引用、且上传超过 24 小时的那些；仍在使用的图片不会被动。',
    pruneRunning: '正在清理…',
    pruneConfirmTitle: '清理未引用的图片？',
    pruneConfirmBody: '没有任何标签引用、且上传超过 24 小时的图片会从服务器上真正删除。没有回收站，此操作无法撤销。仍被标签或打印记录引用的图片不受影响。',
  ...
  presets: {
    ...
    createFailed: '建不了。名称可能重复了，或者所选的标签、打印机已经不在。',
    templateGone: '标签已被删除',
    printerGone: '打印机已被删除',
  },
