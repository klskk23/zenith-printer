...
export const copy: Copy = {
  ...
  status: {
    ...
    remainingSupported: 'Reports remaining stock',
  ...
  dataSources: {
    ...
    nexusTitle: 'Connect to the asset ledger',
    nexusExplain: 'Pick a category. The address and the key are configured by whoever deploys this service, and a row is identified by the ledger own device id — none of that is asked for here.',
    nexusCategory: 'Category',
    nexusName: (fallback: string): string => `Name (blank uses "${fallback}")`,
    nexusColumns: (columns: string[], total: number): string =>
      `${total} rows in this category. Columns: ${columns.join(', ')}`,
    nexusConnect: 'Connect',
    nexusUnreachable: 'Could not reach the asset ledger',
    nexusNoCategories: 'The ledger has no categories yet.',
    addNexus: 'From the asset ledger',
    fromLedger: (category: string): string => `From the asset ledger · ${category}`,
  ...
  templates: {
    ...
    searchPlaceholder: 'Search label names',
    open: 'Open',
    confirmDelete: 'Delete this label? Printed history is unaffected.',
    thumbnailAlt: (name: string): string => `Thumbnail of ${name}`,
    thumbnailMissing: 'This design could not be drawn',
    import: 'Import',
    importFile: 'Choose a label file',
    export: 'Export',
    exportAll: 'Export all',
    importUnreadable: 'This file is not valid JSON and cannot be read. Check that you picked a file produced by Labels → Export all.',
  ...
  settings: {
    ...
    pruneConfirmBody: 'Images that no design refers to and that were uploaded more than 24 hours ago will be deleted from the server. There is no recycle bin and this cannot be undone. Images still used by a label or a print record are not affected.',
