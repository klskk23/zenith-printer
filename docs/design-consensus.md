...
## 6.2 002 阶段推翻/发现的三处（2026-08-21）

完整数据见 `specs/002-web-workspace-editor/research.md`。

3. **「条码模块宽度必须是偶数」这条我们自己定的规则不成立。** 原推理是：
   bwip-js 按中心线描边，奇数宽度的条边缘会落在半点上。实测 7 组码制/内容 ×
   缩放 1/2/3/5/7，**非整数边缘出现 0 次**。

   错误来源是一次测量失误——校验脚本用**第一条 path 的 `stroke-width`** 去算所有条
   的边缘，而 bwip-js 把条按宽度分组成若干 path、各有各的 stroke-width。
   按 path 配对重测后，奇数缩放同样整数对齐。真正的下限是 **2 dot，来自扫描规范
   而非渲染能力**：203 dpi 下即 0.25 mm，Code 128 常用的 X 尺寸。

   > 这条限制曾把可用宽度档位砍掉一半，而且**有测试在保护它**——
   > 一条断言「拒绝奇数模块宽度」的用例。测试通过不代表规则存在。

4. **二维码元素被渲染成一维条码。** `ir-to-svg` 的 `case 'qrcode'` 里硬编码了
   `symbology: 'code128'`，而码制映射表根本没有 `qrcode` 项。声明 15×15 mm
   （120 dot 见方）的二维码，实际画出宽 **486 dot** 的 Code 128——比 400 dot 的
   画布还宽。

   > 前后端共用同一份 `ir-to-svg`，所以**编辑器预览和实际打印一致地错**，
   > 用户看不出异常。这正是它长期未被发现的原因：一致性保证保证的是「两边一样」，
   > 不是「两边都对」。

5. **模板并发控制的时间戳令牌形同虚设。** 保存时比对 `updatedAt`，而写入的也是
   `clock.now()`——同一时钟刻度内的两次保存写出相同时间戳，第二次比对通过，
   先保存者的修改被**静默覆盖并报告成功**。

   > 更要命的是现有的三个并发测试都在保存前调了 `clock.advance(1000)`。
   > 正是那句 advance 让测试通过的——测试在替缺陷打掩护。
   > 已改为单调递增的整数计数器，并补了一个**不推进时钟**的回归用例。

## 6.3 005 阶段推翻的三处（2026-09-21）

完整依据见 `specs/005-nocturne-web-redesign/`（`spec.md` 的「背景与取舍」与 `research.md`）。

6. **「功能以标签页形式打开，切换保留状态」（002 FR-010~013）不再成立。** 原决定的
   全部理由是：切走再切回来，选中元素、缩放与撤销历史还在——所以每个标签页都保持挂载。
   005 先尝试把这个保证搬进本机草稿，用户看过实物后**撤销了草稿体系**：编辑器的未保存
   修改只在它打开期间存在，**离开即放弃**——离开前若有修改，问一次「保留这些修改吗？」。
   新决定：**一次只有一个页面**，地址名之；侧栏常驻；编辑器左上有「返回标签」。

   > 代价记录在案：两张标签不能在应用内并排对照，多开靠浏览器标签页；改到一半必须先保存
   > 才能去别处。这是用户在看过草稿版之后明确选择的更简单的模型。

7. **「保存时比对版本，冲突则拒绝并提示重新加载」（002 Clarification）改为"最后保存为准"。**
   服务器的版本令牌保留——它仍然防止同一瞬间的交错写入——但客户端被 409 拒绝时会取当前
   版本再保存一次，不再要求人先重新载入。按下保存的人是最后一个，就以他为准。

8. **首页、模板库、标签设计三个入口合并为一个「标签」。** 首页是三个列表页的摘要，
   摘要的价值在于"决定去哪"；当去处只剩一个（画廊）时，摘要就成了绕路。打印机与队列
   的状态压成画廊顶部的一条状态带。「模板」这个词从用户面前退出：实体统一叫「标签」，
   打印出来的实物也叫标签，靠量词区分（「3 张」是纸，「货架标签」是定义）。接口字段
   `template*` 不改，以免制造破坏性变更。

   > 对外地址契约不变：`/design/{templateId}?preset=…` 与 `/templates` 继续可用并改写为
   > 新地址（`docs/nexus-assets.md` 不改）。这是本次导航变更算 MINOR 而非 MAJOR 的前提。

## 6.4 006 阶段推翻的五处（2026-09-22）

完整依据见 `specs/006-label-print-flow/`（`spec.md` 的「背景与取舍」与 `research.md`）。

9. **「打印确认是一个对话框」不再成立。** 打印要定四件事——哪台机器、哪套参数、哪几行、
   每行几份——而选行是要翻表格的活，1284 行在对话框里滚不动。拆成两页：**打印**（选）与
   **确认**（看一眼再出纸），各有地址，可刷新、可前进后退。

10. **FR-017 的常驻警示「打印会消耗标签纸且无法撤销」删除。** 一张一直在那儿的警示等于
    没有；「确认」页本身就是那次确认，它有名字、有清单、有一个大数字和一个按钮。

11. **005 刚做的打印头幅宽示意图删除。** 它只在标签比打印头宽时才有话说，而那种情况
    现在由「确认」页清单里的一行文字说明。同一行也覆盖内容被裁切与打印机未探测。
    判断仍来自服务端的 `POST /print-jobs/preflight`——绑定变量的条码宽度随内容变化，
    只有那里能逐行判断。

12. **002 FR-041 的预览保留，并挪到「确认」页。** 一度打算删掉——理由是画布已经回答过
    "这张长什么样"。这个理由只对没绑数据源的标签成立：**条码的宽度跟着内容走**，第 87 行
    可能超界而第 1 行完好，画布上看不出来。所以「确认」页显示第一张（服务端按真实管线
    渲染的二值化位图），并可展开逐张看每个选中的行，每页十张；被裁切的那几张自己标出来。
    没绑数据源时只有一种样子，不给展开。

    > 连带：预检只检查设计本身，所以那行红字不再声称"有几张受影响"，只说超出多少毫米。

13. **`/design/{id}?preset=` 的落点从编辑器改为打印页。** 预设的意思是「打印机、参数、
    份数都给你摆好了」，那是打印的语境。不带 `?preset=` 的 `/design/{id}` 仍落编辑器。
    地址契约不变，`docs/nexus-assets.md` 的描述随之更新。

    > 另记：步条左端不设返回箭头——第 ① 步「标签」就是出口，箭头与它说的是同一件事。
    >
    > 另记：界面上的常驻说明文字自此分三档——原理删除、操作说明收进「?」、错误与不可
    > 撤销操作的确认原位保留。规则写在 `.agents/rules/ux-and-api.md`。
