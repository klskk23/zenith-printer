...
打开后落在这张标签的**打印页**（地址被改写为 `{ZENITH_URL}/labels/{templateId}/print?preset={presetId}`），
打印机、打印参数、每行份数**都已按预设摆好**。绑了数据源的标签还要在这一页勾选要打哪几行。
**不会自动出纸**——按不按由人决定。`?preset=` 留在地址里，刷新和前进后退都不丢。
