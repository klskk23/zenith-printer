...
「从地址接入」要对方回一段这样的 JSON：

```json
{
  "columns": ["id", "序列号", "型号"],
  "rows": [
    { "id": "f3ee54e2", "序列号": "112394521950", "型号": "X100" }
  ],
...
- **所有值都是字符串。** 日期、布尔值由对方渲染成人读得懂的样子——这边不做类型推断，
  否则 `08` 会变成 `8`，印到条码上就是另一个东西。
- `columns` **有序且权威**，每一行的键必须和它完全一致，不多不少。少一个键就是标签上
  一处空白，而空白标签没人在纸用完之前会发现。
- 列名可以是任意合法标识符，**中文可以**。

配置里有三样东西值得说：

**请求头**放凭证（比如 `Authorization: Bearer …`）。存下之后**再也不回显**——列表里只
看得到键名。理由和 Google 私钥只能走环境变量是同一条：这个服务自己没有认证，就不能把
去别处认证的钥匙吐回来。

**key 列**是必填的，它是这类数据源能安全存在的前提。这个产品原本用**位置**作为行的身
份：主键是 `(数据源, 行号)`，每次写入都是整表重排。对「人手改表、改完自己刷、当场就打」
成立；对会自己变的表不成立——上游插一行，下面全体位移，而勾选的行号仍然存在，于是**无
人察觉**，打出来是另一批标签。有了 key 列，刷新按 key 合并：同 key 更新、新 key 追加、
消失的删除，存活的行留在原位。勾选也按 key 走，刷新之后不会被清空。

**自动刷新间隔**默认 0，也就是「只在你点刷新时刷」。大于 0 时，打开设计页或打印页若已
超期就刷一次；**刷新失败不会挡住页面**——显示上一次取到的内容，并说明这次失败了。还有一
个「打印前先刷新」开关，同样默认关，且只有配了 key 列才允许打开。

刷新失败一律不覆盖已有的行。对方系统宕了，这台机器照样能打印。
...
| [`docs/external-systems.md`](external-systems.md) | 给对接方看的一份：行信封、HTTP 数据源、打印预设 |
