...
1. **四步工作流**。标签的四件事各占一步——画廊、设计、打印、确认——由一条常驻步条标位置。打印确认对话框拆成两页：「打印」做选择，「确认」看一眼再出纸。对话框里的三样东西（预览、常驻消耗警示、打印头示意图）去掉；裁切风险改为只在真会发生时，在「确认」页说一行。
...
**Scale/Scope**: 新增 2 个可导航页面（打印、确认）+ 1 条步条；删除 1 个对话框与 3 个它专属的组件；清理约 20 处文案
...
│   ├── selection.ts           # 沿用
│   ├── print-dialog.tsx       # 删
│   ├── preview.tsx            # 删（FR-040）
│   ├── use-label-preview.ts   # 删（只被 preview 用）
...
`POST /print-jobs/preflight` 的 `warnings` 是唯一能逐行判断的来源（绑定变量的条码宽度随内容变化）。「确认」页进入时调用一次，把 `warnings` 连同标签宽度与打印机能力交给 `flow.ts` 的 `clipSummary()`，得出一行文字或什么都不显示。预检失败不阻止提交（既有行为）。
...
3. **US2 设计页衔接**：顶栏收成 ← + 步条，撤销/重做进画布头部，底条保存/另存为/继续
