...
|---|---|---|
| `id` | TEXT | 主键 |
| `name` | TEXT | **唯一**，**可随时更改**（FR-019/FR-019a）。仅为供人辨认的标签；标识是 `id` |
...
| `dataSourceId` | TEXT NULL | 所绑数据源的 **id**。一个设计最多一个（FR-029）——这里是那个「一」的落点。数据源被删后此值悬空，由 FR-028a 的警告标记兜住，**不设外键级联清空** |
...
`Template.dataSourceId` **刻意不设外键级联**：数据源被删后此值悬空，而悬空正是要被
看见的状态——由 `bindingIssue` 在读取时算出，在模板列表与设计页上显示为警告标记
（FR-028a）。级联清空会把「这个设计原本绑着一张已经没了的表」变成「这个设计从来没绑
过表」，两者对用户是完全不同的处境。

**列名**与模板之间同样没有外键：它写在内容字符串里，是文法而非关系。改名不受影响，
因为绑定按 id；删除也不被拦截，因为重新绑到另一张同形状的表即可复原（FR-028）。
