...
| | |
|---|---|
| `zenith.db` | 打印机、模板、数据源、任务历史，**以及标签里用到的图片** |
| `zenith.db-wal` / `-shm` | WAL 模式的伴随文件。服务正常停止时会 checkpoint 进主库 |

图片自迁移 15 起存在库里(BLOB),不再有 `uploads/` 目录。老库首次启动时会把文件搬进去,
**并且不删原文件**——那一步交给你,等确认图片都还在之后自己 `rm -rf`。
...
> **图片在库里**（迁移 15 起）。曾经它们是磁盘上的文件加一行指针，而那套安排的每一个麻烦
> 都是同一个麻烦——两样东西要保持同步：路径换台机器就没意义、有文件没行、有行没文件、备份
> 只有在有人记得那第二个目录时才完整。收成一处之后，这些问题问都问不出来。
>
> 但 **`-wal` 那条仍然成立**：字节进了库不等于拷一个文件就够，最近的写入还在
> `zenith.db-wal` 里。停服务（或 checkpoint）那一步没有被省掉，被省掉的是第二个目录。
...
被删掉的模板，都会留下一行——而界面上从来不会提到它们，这正是它们只增不减的原因。
...
「磁盘上有文件、库里没行」这一类垃圾随着迁移 15 消失了——只有一处存储，也就无所谓不同步。

但**行的垃圾没有消失，而且代价变大了**：每一次被丢弃的粘贴仍然留下一行，而那一行现在装着
图片本身。一张废弃的 4MB 图从"躺在某个目录里、`du` 至少找得到"变成了"把数据库撑大、也把
每一次备份撑大"。所以这个清理不是可以删掉的旧代码,是比以前更该定期跑的东西。
